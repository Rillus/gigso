    ...
    constructor() {
        ...
        const styles = `
            ...
            @media print {
                ...
                .verse, .chorus, .bridge, .chord-diagrams {
                    page-break-inside: avoid;
                }
                
                /* Ensure chord diagrams stay together */
                .chord-diagram {
                    page-break-inside: avoid;
                    background: transparent !important;
                }
    ...
    formatLyrics(lyrics, chordPositions) {
        ...
        lines.forEach(line => {
            ...
            if (chordPositions && chordPositions[trimmedLine]) {
                ...
                positions.forEach(pos => {
                    result += processedLine.slice(lastIndex, pos.position);
                    result += `<span class="chord-above">${pos.chord}</span>`;
                    lastIndex = pos.position;
                });
                result += processedLine.slice(lastIndex);
                processedLine = result;
            }
            
            formattedLyrics += `<p>${processedLine}</p>`;
        });
