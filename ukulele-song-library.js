export default class UkuleleSongLibrary {
    static songs = [
        {
            id: 'dont-stop-believin',
            title: 'Don\'t Stop Believin\'',
            artist: 'Journey',
            key: 'C',
            difficulty: 'beginner',
            genre: 'rock',
            tempo: 120,
            chordProgression: ['C', 'Am', 'F', 'G'],
            strummingPattern: 'D-D-U-U-D-U',
            strummingNotes: 'Down-Down-Up-Up-Down-Up pattern, repeat for each chord',
            tags: ['beginner-friendly', 'practice'],
            lyrics: `
[Intro]
{C}   {G}   {Am}   {F}
{C}   {G}   {F}    {F}

[Verse 1]
{C}   Just a {G}small town girl,{Am}  livin' in a {F}lonely world
{C}   She took the {G}midnight train going {F}anywhere{F}
{C}   Just a {G}city boy,{Am}    born and raised in {F}South Detroit
{C}   He took the {G}midnight train going {F}anywhere

[Instrumental - same as Intro]

[Verse 2]
{C}A singer in a {G}smoky room, {Am}the smell of wine and {F}cheap perfume
{C}  For a smile they can {G}share the night, it goes {F}on and on {F}and on and on

[Chorus]
{F}Strangers waitin'   {C}up and down the boulevard
Their {F}shadows searchin' in the {C}night
{F}Streetlights, people, {C}livin' just to find emotion
{F}Hidin', somewhere in the {G}ni{C}ght {G}  {C}  {F}

[Break]
{C}   {G}   {Am}   {F}

[Verse 3]
{C}   Workin' hard to {G}get my fill,{Am}  everybody {F}wants a thrill
{C}   Payin' anything to {G}roll the dice, just {F}one more {F}time
{C}   Some'll win, {G}some will lose,{Am}  some are born to {F}sing the blues
{C}   Oh, the movie {G}never ends; it goes {F}on and on and {F}on and on

[Chorus]

[Instrumental - same as Intro]

[Outro]
{C}Don't stop be{G}lievin', {Am}hold on to that {F}feelin'
{C}Streetlights, {G}people, oh-oh-{F}o-----{F}--h
{C}Don't stop be{G}lievin', {Am} hold o-{F}----n
{C}Streetlights, {G}people, oh-oh-{F}o----{F}---h
{C}Don't stop be{G}lievin', {Am}hold on to that {F}feelin'
{C}Streetlights, {G}people, oh-oh-o{F}----{F}--h`
        },
        {
            id: 'with-or-without-you',
            title: 'With or Without You',
            artist: 'U2',
            key: 'C',
            difficulty: 'beginner',
            genre: 'rock',
            tempo: 120,
            chordProgression: ['C', 'G', 'Am', 'F', 'C', 'G', 'F', 'C'],
            strummingPattern: 'D-D-U-D-U-D-U',
            strummingNotes: 'Gentle strumming pattern, emphasize the downbeats',
            tags: ['ballad', 'slow-tempo', 'emotional'],
            lyrics: `
[Intro]
{C}
{C}   {G}    {Am}   {F}
{C}   {G}    {Am}   {F}

[Verse 1]
{C}    See the {G}stone set in your {Am}eyes
See the {F}thorn twist in your {C}side
I {G}wait for you{Am}    {F}

[Verse 2]
{C}    Sleight of {G}hand and twist of {Am}fate
On a bed of {F}nails she makes me {C}wait
And I {G}wait with{Am}out yo{F}u 

[Chorus]
With or with{C}out you {G}
With or {Am}without you {F}

[Verse 3]
{C}     Through the {G}storm we reach the {Am}shore
You give it {F}all but I want {C}more
And I'm {G}waiting for {Am}you {F}

[Chorus]
With or with{C}out you {G}
With or {Am}without you {F}
I can't {C}live
{G}With or with{Am}out you {F}

[Bridge]
{C}  {G}  {Am}   {F}

[Verse 4]
And you {C}give yourself aw{G}ay
And you {Am}give yourself aw{F}ay
And you {C}give
And you {G}give
And you {Am}give yourself {F}away

[Verse 5]
{C}   My hands are {G}tied  {Am}
My body {F}bruised, she's got me with
{C}  Nothing to {G}win and
{Am}Nothing left to {F}lose

[Verse 6]
And you {C}give yourself aw{G}ay
And you {Am}give yourself aw{F}ay
And you {C}give
And you {G}give
And you {Am}give yourself {F}away

[Chorus]

[Outro]
{C}  {G}  {Am}   {F}
{C}  {G}  {Am}   {F}
With or with{C}out you {G}
With or {Am}without you {F}
I can't {C}live
{G}With or with{Am}out you {F}
`
        },
        {
            "id": "im-yours",
            "title": "I'm Yours",
            "artist": "Jason Mraz",
            "key": "C",
            "difficulty": "beginner",
            "genre": "Pop",
            "tempo": 120,
            "chordProgression": [
              "C",
              "G",
              "Am",
              "F",
              "D",
              "Em"
            ],
            "strummingPattern": "D-D-U-U-D-U",
            "strummingNotes": "Adjust tempo and dynamics as needed",
            "tags": [
              "beginner-friendly"
            ],
            "lyrics": "[Verse 1]\nWell {C}you done done me in, you bet I felt it\nI t{G}ried to be chill but you're so hot that I melted,\nI {Am}fell right through the cracks,\n{F}And now I'm tryin' to get back...\nBefore the c{C}ool done run out, I'll be givin' it my bestest\nAnd no{G}thing's going to stop me but divine intervention\nI re{Am}ckon it's again my turn,\nTo w{F}in some or learn some...\n\n[Chorus 1]\n{C}But I won't hesit{G}ate\nNo more, no {Am}more\nIt can not w{F}ait, I'm yours\n{C}   {G}   {Am}   {F}\n\n[Verse 2]\n{C}Well open up your mind and see like {G}me\nopen up your plans and then you're fr{Am}ee\nlook into your heart and you'll find {F}love love love\n{C}Listen to the music of the moment come and sing/dance with {G}me\nWe're just one big f{Am}amily\nAnd it's our God-forsaken right to be l{F}oved loved loved loved l{D}oved\n\n[Chorus 2]\n{C}So I won't hesit{G}ate\nNo more, no {Am}more\nIt can not w{F}ait, I'm sure\nThere's no n{C}eed to complic{G}ate\nOur time is sh{Am}ort\nThis is our f{F}ate, I'm yours\n\n[Adlib]\n{C}  {G}  {Am}  {Em}  {F}     {D}     x2\n\n[Verse 3]\nI've {C}been spending way too long checking my tongue in the mirror\nAnd{G} bending over backwards just to try to see it clearer\nBu{Am}t my breath fogged up the glass,\n{F}And so I drew a new face and I laughed\nI guess what{C} I be saying is there ain't no better reason\nTo rid{G} yourself of vanity and just go with the seasons\nIt's{Am} what we aim to do\nOur {F}name is our virtue\n\n[Chorus 1]\n{C}But I won't hesi{G}tate\nNo more, no {Am}more\nIt cannot wa{F}it, I'm yours\n{C}   {G}   {Am}   {F}\n\n[Verse 4]\n{C}Well, open up your mind and see like{G} me\nOpen up your plans and damn you're fr{Am}ee\nLook into your heart and you'll find {F}the sky is you{D}rs\n\n[Outro]\n{C}So please don't—{G}please—don't—please don't—\nThere's no n{C}eed to complic{G}ate\n'Cause our tim{Am}e is short\nThis o—this o{F}—this is our fate, I'm yours"
        },
        {
            "id": "three-little-birds",
            "title": "Three Little Birds",
            "artist": "Bob Marley",
            "key": "C",
            "difficulty": "beginner",
            "genre": "Reggae",
            "tempo": 148,
            "chordProgression": [
              "C",
              "F",
              "G"
            ],
            "strummingPattern": "D-D-U-U-D-U",
            "strummingNotes": "Adjust tempo and dynamics as needed",
            "tags": [
              "beginner-friendly"
            ],
            "lyrics": "[Chorus]\nDon't wor{C}ry, about a thing\n'Cause {F}every little thing, gonna be al{C}right\nSingin' don't w{C}orry, about a thing\n'Cause {F}every little thing, gonna be al{C}right\n\n[Verse]\nRise up this m{C}ornin'\nSmile with the {G}rising sun\nThree little {C}birds sit by my {F}doorstep\nSingin' {C}sweet songs\nOf melodies {G}pure and true\nSingin',{F} this is my message to {C}you-ou-ou"
        },
        {
            "id": "stand-by-me",
            "title": "Stand By Me",
            "artist": "Ben E King",
            "key": "C",
            "difficulty": "beginner",
            "genre": "folk",
            "tempo": 120,
            "chordProgression": [
              "C",
              "Am",
              "F",
              "G"
            ],
            "strummingPattern": "D-D-U-U-D-U",
            "strummingNotes": "Adjust tempo and dynamics as needed",
            "tags": [
              "beginner-friendly"
            ],
            "lyrics": "[Verse 1]\nWhen the nigh{C}t has come\n{Am}And the land is dark\nAnd the moon{F} is the on{G}ly light we'll {C}see\nNo, I won'{C}t be afraid\nNo, I {Am}won't be afraid\nJust as lon{F}g as you stan{G}d, stand by {C}me\n\n[Chorus]\nSo, darling, darling, {C}stand by me\nOh, {Am}stand by me\nOh, {F}stand,{G} stand by me{C}\nStand by me\n\n[Verse 2]\nIf the sk{C}y that we look upon\n{Am}Should tumble and fall\nOr the mount{F}ain should {G}crumble to the {C}sea\nI won't cr{C}y, I won't cry\nNo, I w{Am}on't shed a tear\nJust as lon{F}g as you stan{G}d, stand by {C}me\n\n[Chorus]\n\n[Instrumental]\n{C}    {Am}   {F}  {G}  {C}    x2\n\n[Chorus]\n\n[Chorus]\nWhenever you're in trouble won't you {C}stand by me, oh, s{Am}tand by me\nOh, s{F}tand now, oh, s{G}tand, stand by{C} me"
        },
        {
            "id": "leaving-on-a-jet-plane",
            "title": "Leaving on a Jet Plane",
            "artist": "John Denver",
            "key": "G",
            "difficulty": "intermediate",
            "genre": "folk",
            "tempo": 120,
            "chordProgression": [
              "G",
              "C",
              "D"
            ],
            "strummingPattern": "D-D-U-U-D-U",
            "strummingNotes": "Adjust tempo and dynamics as needed",
            "tags": [
              "beginner-friendly"
            ],
            "lyrics": "[Verse 1]\nAll my {G}bags are packed, I'm {C}ready to go\nI'm {G}standing here out{C}side your door\n{G}I hate to wake you u{C}p to say goodb{D}ye\nBut the {G}dawn is breaking, it's {C}early morn,\nThe {G}taxi's waiting, he's {C}blowing his horn\n{G}Already I'm so l{C}onesome I could d{D}ie\n\n[Chorus]\nSo {G}kiss me and {C}smile for me\n{G}Tell me that you'll {C}wait for me\n{G}Hold me like you'll {C}never let me g{D}o\nI'm {G}leaving {C}on a jet plane\n{G}Don't know when I'l{C}l be back again\n{G}Oh bab{C}e, I hate to go{D}\n\n[Verse 2]\nThere's so{G} many times I've {C}let you down\nS{G}o many times I've {C}played around\n{G}I tell you now t{C}hey don't mean a t{D}hing\nEvery {G}place I go I'll {C}think of you\nEvery {G}song I sing I'll {C}sing for you\nWhen {G}I come back I'{C}ll bring your wedding {D}ring\n\n[Chorus]\n\n[Verse 3]\n{G}Now the time has {C}come to leave you\n{G}One more time {C}let me kiss you\nThen {G}close your eyes {C}I'll be on my {D}way\n{G}Dream about the {C}days to come\nWhen {G}I won't have to {C}leave alone\nA{G}bout the times {C}I won't have to {D}say\n\n[Chorus]"
        }
    ];
}